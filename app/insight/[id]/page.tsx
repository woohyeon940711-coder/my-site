...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const article = ARTICLES.find((a) => a.id === Number(id) && a.category === "insight");
  if (!article) return {};
  const description = article.summary.slice(0, 155) + (article.summary.length > 155 ? "…" : "");
  return {
    title: `${article.title} | CannabisInsight`,
    description,
    keywords: ["대마 인사이트", "헴프 분석", "CBD 시장", "카나비스인사이트", "CannabisInsight"],
    openGraph: {
      title: `${article.title} | CannabisInsight`,
      description,
      url: `https://www.cannabisinsight.kr/insight/${article.id}`,
    ...
    alternates: { canonical: `https://www.cannabisinsight.kr/insight/${article.id}` },
  };
}
